...
export async function startPreferredNativeAudio(){
  const saved=await nativeAudioPreferences();
  try{
    return await startNativeAudio(Boolean(saved.prefer_asio),saved.input_device??null,saved.output_device??null);
  }catch(error){
    const hadSavedRoute=Boolean(saved.prefer_asio||saved.input_device||saved.output_device);
    if(!hadSavedRoute)throw error;
    return startNativeAudio(false,null,null);
  }
}
